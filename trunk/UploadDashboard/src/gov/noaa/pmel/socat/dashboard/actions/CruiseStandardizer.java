...
public class CruiseStandardizer {
	 ...
	 * 		if the PI name(s) in the DSG file is/are not recognized, or
	 * 		if the PI name(s) are too long for the DSG or decimated DSG file
	 * @throws IOException 
	 * 		if there are problems reading or updating the DSG file
	 * 		or decimated DSG file.
	 */
	public void standardizePINames(String expocode) 
		...
		String piNames = mdata.getScienceGroup();
		...
		if ( newPiNames.equals(piNames) )
			return;
		// Update the full-data DSG file
		dsgFile.updatePINames(newPiNames);
		// Update the decimated-data DSG file
