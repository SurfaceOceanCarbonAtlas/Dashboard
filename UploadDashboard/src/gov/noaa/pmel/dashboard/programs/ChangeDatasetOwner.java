...
public class ChangeDatasetOwner {
	...
	public static void main(String[] args) {
		if ( args.length != 1 ) {
			...
			System.err.println("has a space-separated dataset and new owner dashboard username.  The ");
			...
			try {
				...
				try {
					...
					while ( dataline != null ) {
						...
						if ( tokens.length != 2 ) {
							System.err.println("Unable to get dataset and owner from: " + dataline);
							System.exit(1);
						}
						...
						try {
							String upperExpo = DashboardServerUtils.checkDatasetID(tokens[0]);
							if ( exposOwners.put(upperExpo, tokens[1]) != null ) {
								System.err.println("More than one owner specified for " + upperExpo);
								System.exit(1);
							}
						} catch (Exception ex) {
							System.err.println("Invalid dataset given in: " + dataline);
							System.exit(1);
						}
