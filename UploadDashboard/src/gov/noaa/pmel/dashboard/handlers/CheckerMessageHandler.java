	...
	public void processCruiseMessages(DashboardCruiseWithData cruiseData, 
		...
		try {
			...
			for ( Message msg : output.getMessages() ) {
				...
				TreeSet<Integer> colNums = msg.getColumnIndices();
				if ( colNums.size() > 1 )
					throw new RuntimeException("more than one column number for a message "
							+ "in CheckerMessageHandler: " + colNums.toString() );
				Integer colNum;
				if ( colNums.size() == 1 ) {
					colNum = colNums.first();
					mappings.add(SCMSG_COLUMN_NUMBER_KEY + SCMSG_KEY_VALUE_SEP + colNum.toString());
				}
				else {
					colNum = Integer.valueOf(-1);
				}

				TreeSet<String> colNames = msg.getColumnNames();
				if ( colNames.size() > 1 )
					throw new RuntimeException("more than one column name for a message "
							+ "in CheckerMessageHandler: " + colNames.toString() );
				if ( colNames.size() == 1 ) {
					mappings.add(SCMSG_COLUMN_NAME_KEY + SCMSG_KEY_VALUE_SEP + colNames.first());
				}
