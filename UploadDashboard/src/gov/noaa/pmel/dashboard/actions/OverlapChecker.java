 ...
 * and time values either within a data set or between any two data sets. 
 * Extensive overlaps are very likely to be erroneous duplication of data, 
...
public class OverlapChecker {
	...
	/**
	 * Create a overlap checker which gets data from the full-data DSG files 
	...
	/**
	 * Checks for overlaps between a data set and a collection of other data 
	 * sets.  Reads the data once for the primary data set, then reads the 
	 * data as needed for the other data sets.  Ignores any data points with a 
	 ...
	 * @return
	 * 		the list of overlaps found; never null but may be empty.
	...
	public double[][] getMaskedLonLatTimeVals(String upperExpo) 
		...
		for (int k = 0; k < dataflags.length; k++) {
			if ( DashboardUtils.WOCE_BAD.equals(dataflags[k]) ) {
	...
	/**
	 * Checks for overlaps between two datasets. 
	 * If the two expocodes are the same, this detects overlaps 
	 * within a dataset (excludes matching a data point with itself).
	 * Assumes the data points are ordered in increasing time.
	 * 
	 * @param expocodes
