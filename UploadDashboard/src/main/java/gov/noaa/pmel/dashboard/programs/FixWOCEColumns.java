            ...
            try {
                        ...
                        if ( woceWaterColName.equals(userColNames.get(k)) ) {
                            ...
                            if ( !DashboardUtils.OTHER.typeNameEquals(dataColTypes.get(k)) )
                                throw new IllegalArgumentException(
                                        "WOCE CO2 water column does not have type OTHER (colNum = " +
                                                Integer.toString(k + 1) + ", type = " + dataColTypes.get(k)
                                                                                                    .getDisplayName() + ")\n" +
                                                "No changes made to this dataset.");
                            if ( colFound )
                ...
                if ( !woceAtmColName.isEmpty() ) {
                    ...
                    for (int k = 0; k < numCols; k++) {
                        if ( woceAtmColName.equals(userColNames.get(k)) ) {
                            ...
                            if ( !DashboardUtils.OTHER.typeNameEquals(dataColTypes.get(k)) )
                                throw new IllegalArgumentException(
                                        "WOCE CO2 atm column does not have type OTHER (colNum = " +
                                                Integer.toString(k + 1) + ", type = " + dataColTypes.get(k)
                                                                                                    .getDisplayName() + ")\n" +
                                                "No changes made to this dataset.");
                            if ( colFound )
                ...
                if ( !woceFlagSet.isEmpty() ) {
                    ...
                    try {
                        ...
                        throw new IllegalArgumentException(
                                "Unable to add a WOCE flag to the database: " + ex.getMessage() +
