...
public class CruiseModifier {
        ...
        DataInfo(String expocode, int num, char regionID, Double sectime, Double latitude,
            ...
            if ( num <= 0 )
                throw new IllegalArgumentException(
                        "invalid row number of " + Integer.toString(num) + " for " + expocode);
            this.num = num;
     ...
     * @return true is any WOCE flags in the DSG files changed;
     * otherwise, false
     * @throws IllegalArgumentException
     *         if the expocode is invalid
     * @throws SQLException
                ...
                for (DataLocation dataLoc : woceEvent.getLocations()) {
                    ...
                    while ( rowNum < numData ) {
                        if ( DashboardUtils.closeTo(times[rowNum], woceLocTime, 0.0, maxTimeDiff) &&
                                ...
                                ( ( dataVals == null ) ||
                                        DashboardUtils.closeTo(dataVals[rowNum], woceLocDataValue, 1.0E-5,
                                                               maxValueDiff) ) ) {
                            matchFound = true;
                            break;
                        }
     ...
     * @return list of messages describing the duplicate lon/lat/time/fCO2_rec
