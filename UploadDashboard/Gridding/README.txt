...
0. First look at all the scripts and makes sure the version number, dates, 
   ...
   The script define_dates_v2020.jnl is called by scripts to set up the monthly 
   time axis. Run it and check the axis. Check the count for data in the 
   triples file. Also update the shell scripts (step 4) for the version number
   and date of release.

1. Run triples2nc.jnl to convert the tsv file into a netCDF file 
   Make sure that the variables this scrit defines and writes to the nc file
...
4. (We no longer do this...) Run make_icoads_datasets.jnl to make the local copies of icoads
   data used in the LAS.
